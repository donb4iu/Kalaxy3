...
The implemented controller is `scripts/sage/workflows/routine_git_lifecycle.py`, invoked only through the repository-owned CLI `scripts/sage/sage-routine-git-lifecycle.py` from a checksum-bound `operator.git-proposal@1.3.0` `routine-git-lifecycle` boundary using command-proposal schema 1.2. Before mutation it independently verifies the active request state, exact proposal command digest, feature branch and HEAD, declared path scope, pass-only validation receipts, synchronized upstream and remote feature branch, and frozen `origin/main`.
One operator execution then authorizes exactly stage → commit → push through `git.repository.commit_and_push`. The controller records a local checksum-bound receipt and uses `git.inspect` to prove the resulting single-commit topology, exact path delta, clean tree, upstream equality, and remote feature-branch identity. The same repository-owned CLI immediately feeds that receipt into request execution; the routine path has `pasted_output_required=false` and `repository_receipt_required=true` and never asks the caller to author an internal result envelope.
Request execution binds the receipt to the active proposal and independently repeats the post-mutation Git proof before metrics and evidence closeout. Existing pasted stage → commit → push continuation remains only for compatibility with already-open/manual states and genuinely external boundaries. During the one-time schema-1.2 activation edge, an already-open checksum-bound `routine-git-lifecycle` proposal created under schema 1.0 may be executed by the repository-owned controller and closed from its stronger receipt; proposal generation itself never falls back and all newly generated routine proposals remain schema 1.2. New ordinary validated request execution uses the one-approval routine controller and fails closed when its authority prerequisites are unavailable or changed.
