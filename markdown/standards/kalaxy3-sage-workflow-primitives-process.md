...
`git.safety-guardrail@1.2.0` makes that exception explicit and narrow. The exact routine Git lifecycle controller path may import `GitRepository` and call only `commit_and_push`; branch creation, fetch, direct Git commands, GitHub mutation, credential inheritance, deployment mutation, and the same code at any other path still fail closed. This prepares the repository for a bounded one-approval stage → commit → push composition without weakening the downloaded-helper boundary or authorizing autonomous mutation.
