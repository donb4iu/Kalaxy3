...
9. Generate the canonical planning source through repository-owned `write_source_package`, embedding the exact semantic-understanding and Architect semantic-confirmation artifacts plus their confirmed context dispositions. The default immutable planning-source filename is scoped by both action ID and the exact semantic-confirmation digest, so multiple confirmed slices under one accepted action cannot collide. Explicit `--output` remains a bounded recovery/testing override; `write_source_package` retains fail-closed no-overwrite behavior.
