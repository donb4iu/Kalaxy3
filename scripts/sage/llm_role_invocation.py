"""Fresh, machine-neutral LLM role invocation with an Ollama HTTP reference adapter."""
...
def stable_json(value: Mapping[str, Any]) -> str:
    """Serialize a mapping deterministically for evidence."""
    return json.dumps(dict(value), indent=2, sort_keys=True) + "\n"


def sha256_json(value: Mapping[str, Any]) -> str:
    """Return stable SHA-256 identity for a JSON object."""
    raw = json.dumps(
        dict(value),
        sort_keys=True,
        separators=(",", ":"),
...
def invoke_ollama_json(
    *,
    ...
    try:
        ...
        raise RoleInvocationError("Ollama response did not contain a JSON role result") from error
...
def new_state_dir(prefix: str) -> Path:
    ...
    path = STATE_ROOT / (
        datetime.now().strftime("%Y%m%d-%H%M%S-%f") + "-" + prefix
    )
